### Fixed
- `tool check-links`: scan `.markdown` files and match Markdown extensions case-insensitively like the build (`post.MD` is scanned, resolves `/post/`, and counts toward pagination bounds), and resolve `@/` links against exact content paths.
