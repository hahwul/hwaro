...
describe "Hwaro::Services::CHECK_GROUPS" do
  ...
  it "declares every issue id the Doctor service emits" do
    # The service is split across doctor.cr and doctor/*.cr; scan them all.
    root = File.expand_path("../../../src/services", __DIR__)
    sources = [File.join(root, "doctor.cr")] + Dir.glob(File.join(root, "doctor", "*.cr")).sort!
    source = sources.join("\n") { |path| File.read(path) }
