...
private def value_of(expr : String) : String
  css = compile_source(".probe { v: #{expr}; }")
  match = css.match(/v: (.*);/)
  match ? match[1] : css
end
...
describe "Sass dart parity fixes" do
  ...
  describe "review-pass regressions" do
    ...
    it "replaces every occurrence in selector.replace" do
      compile_source(%(@use "sass:selector";\n.a { r: selector.replace(".b .b", ".b", ".c"); }))
  ...
  describe "second review-pass regressions" do
    ...
    it "emits null !important without a double space" do
      compile_source(".a { y: null !important; }").should contain("y: !important;")
    end
