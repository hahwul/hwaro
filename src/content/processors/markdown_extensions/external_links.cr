  ...
  module Content
    module Processors
      module MarkdownExtensions
        ...
        EXTERNAL_HREF_RE   = /(?<![\w-])href\s*=\s*(?:"https?:\/\/|'https?:\/\/)/i
        EXISTING_TARGET_RE = /(?<![\w-])target\s*=/i
        EXISTING_REL_RE    = /(?<![\w-])rel\s*=\s*(["'])(.*?)\1/i
        ...
        def postprocess_external_links(html : String, config : Models::MarkdownConfig) : String
          ...
          # Raw HTML may preserve uppercase attribute names/schemes and either
          # quote style. Reuse the case-insensitive matcher so the precheck
          # can't skip an anchor the rewrite below would handle.
          return html unless html.matches?(EXTERNAL_HREF_RE)
          ...
          html.gsub(EXTERNAL_ANCHOR_RE) do |match|
            ...
            if rel_match = new_attrs.match(EXISTING_REL_RE)
              ...
              new_attrs = new_attrs.sub(EXISTING_REL_RE, %(rel=#{quote}#{merged.join(' ')}#{quote}), backreferences: false)
            else
