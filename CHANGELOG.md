...
- `hwaro doctor`'s `title-default` advisory fires on `My Hwaro Site`, the placeholder every `hwaro init` writes. It only matched `Hwaro Site` — the internal fallback for an absent `title` — so the check never fired on a real scaffolded site and the placeholder shipped into `<title>`, OG tags and feeds
