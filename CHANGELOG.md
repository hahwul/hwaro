...
- Versioned documentation: `[versions]` + `[[versions.list]]` publish several doc versions side by side (directory-based, modeled on languages) with `page.version`, `page.version_links` and a global `versions` for switchers; latest-at-root or prefixed URLs, version-scoped prev/next, breadcrumbs, sections, menus, related posts and taxonomies, canonical + noindex for older versions, and `search`/`feeds`/`taxonomies` switches for discovery surfaces
