...
### Fixed (Sass)
- A whole-token `url($v)` / `url(ns.$img)` substitutes the variable instead of shipping the sigil literally into the stylesheet (`$` is not valid in a raw CSS URL); a `$` embedded in a larger url token (`url(plain$x.png)`, `url($h:8080/x)`) is valid plain CSS and passes through byte-identical where dart-sass errors; custom properties keep `$` literal, as dart does
...
- Second review pass (15 verified findings, all fixed): `min(0 / 0, 8)` no longer crashes the build with an unlocated comparison error (NaN declines to verbatim); `@supports (width: calc(…))` preserves `calc()` verbatim (folding flipped which browsers matched); `font: calc(16px)/1.5` ships verbatim again (byte-compat) instead of becoming a spaced slash list; `calc(1px+2px)` (no whitespace — invalid CSS, dart errors) declines the fold instead of silently activating a dropped declaration; calc `clamp()` bounds win at exact boundaries with their own unit spelling and mixed unitless/united clamp declines; `Number.format` prints ≥1e15 magnitudes as zero-padded digits (was `inf` near Float64::MAX and off-by-one at 2^53-1); `y: null !important` emits `y: !important` (was a double space); `selector.replace` replaces with dart's pseudo-last compound ordering and errors when the replacement can't unify (was returning the target intact); `selector.parse` accepts its documented `$selector` keyword; constructed `list.slash` values refuse math and keep structural parens inside space lists where lazy `a / b` pairs stay bare; `math.$epsilon`/`$min-number`/`$max-number` store round-trip spellings
...
### Fixed
- `{{ pwa_tags }}` escapes the values it interpolates. A `theme_color` containing a double quote closed the `content="…"` attribute early and the rest of the value became live markup in the `<head>` of every page; the service-worker URL is now emitted as a JS string literal (HTML entities are not decoded inside `<script>`, so escaping it as HTML would have corrupted the path instead)
- Scaffold search overlay: a query matching late in a long title no longer ranks below a content-only match. The title bonus was a flat `100 - titleIdx`, which goes negative past column 100, inverting the title-beats-body order the ranking exists to enforce (`static/js/search.js` and the book scaffold's `book.js`)
...
- **`hwaro tool export` dropped every tag and category on a site that uses `[taxonomies]`.** Neither Hugo nor Jekyll reads taxonomy membership from a nested table, so the export produced an inert `taxonomies` param and a re-import lost the terms outright. The table is now flattened to top-level `tags` / `categories` / … keys, with an explicitly declared top-level key winning (the build's own precedence)
- `hwaro tool check-links` no longer reports the build's own generated routes as dead when run before `hwaro build` — the lint-then-build order CI uses. `/sitemap.xml`, `/robots.txt`, `/llms.txt`, the search index, `404.html` and feeds (including per-section and per-language copies) are resolved from `config.toml`, and `/posts/page/2/` resolves for a section that declares `paginate_by`
- `hwaro tool stats` on a missing content directory exits `HWARO_E_CONTENT` instead of printing "not found" on stderr, a zeroed report on stdout, and exiting 0 — a script could not tell "no content" from a mistyped `--content-dir`. Matches `tool list` / `tool validate`
- `hwaro tool convert` says why it skipped a file. A bulk run reported a bare "N skipped"; the summary now breaks it down (`1 already TOML, 1 not front matter`), and the content-protective skip — a leading `---` pair holding prose rather than front matter — is always named on stderr
- `hwaro tool platform` / `hwaro tool ci` raise classified usage errors (`HWARO_E_USAGE`, exit 2, with a `Did you mean …?` suggestion for a mistyped platform) instead of an unclassified message and exit 1 — the same status their "file already exists" refusal used, so a typo and a real failure were indistinguishable
- `hwaro tool validate` no longer drops `false` front-matter values: `elsif b = value.as_bool?` treated the returned `false` as a failed match, so a YAML/JSON `draft: false` never reached the checks
- **`hwaro deploy` wrote and deleted *outside* the destination directory.** The destination walk followed symlinks, so a link left in the deploy target reached straight through it: `out/index.html -> ../secrets.txt` was overwritten with page markup, and `out/sub -> /data/sub` made every file behind the link a delete candidate — real files outside the deploy root were unlinked, at exit code 0. The sync now treats a destination symlink as a leaf: one standing where a file or directory belongs is replaced with the real thing, a stale one is unlinked, and neither case reads or deletes through it. The empty-directory sweep no longer expands `**` through symlinks either
- **`hwaro deploy` before `hwaro build` wiped the destination.** An empty source directory produced an empty desired set and a delete pass covering everything at the destination, bounded only by the 256-file `max_deletes` default — enough to lose a small site silently. An empty source is now refused (`HWARO_E_CONFIG`), as is a source that selects no files at all while the destination still holds some (`HWARO_E_USAGE`); `--force` clears a destination deliberately
- `path = "~/public"` (and `--source ~/dist`) deploy to the home directory instead of creating a directory literally named `~` inside the project
- `hwaro deploy --target NAME` works. The flag is documented in `features/deployment.md` and is the form Hugo users reach for, but only positional targets were parsed, so it failed with `Invalid option: --target`. It is repeatable and composes with positional names
- `hwaro deploy --dry-run` fails the way a real deploy would: the plan skipped the destination-path and `stripIndexHTML` validation, so `--dry-run --json` reported a clean plan for a destination a real deploy refuses with `HWARO_E_IO`
- A single-slash scheme typo (`url = "s3:/bucket"`) raises the unsupported-scheme error instead of quietly deploying to a *local* directory named `s3:`; `s3://` and `gs://` URLs with no bucket are rejected the same way `az://` already was. Windows drive letters (`C:\out`) remain local paths
- `hwaro deploy --json --confirm` no longer writes a confirmation prompt into the JSON document — JSON mode is non-interactive and fails with the same actionable usage error non-TTY runs already got
- `deployment.workers` and duplicate `[[deployment.targets]]` names now warn instead of looking applied: workers is never used by the built-in sync, and only the first target of a duplicated name is reachable
...
- **`hwaro doctor` reported "no issues found" on a site with warnings.** The inline ✓/⚠ board is rendered from a registry three diagnostics never joined, so `menu-parent-undefined`, `menu-undeclared` and `structure-missing-index` printed in the detail list under an all-green board. A spec now pins every id the service emits to the registry
- `hwaro doctor` no longer reports a passing check for a scan that never ran: with `templates/` missing, `template syntax` rendered ✓ even though `check_templates` bails out before parsing anything. Blocked checks render as `(skipped)`, as the config group's already did
- **`hwaro doctor` flagged ordinary multilingual page bundles as broken sections.** A folder holding `index.md` + `index.ko.md` was read as a section carrying loose markdown, so every such bundle drew "Section directory missing `_index.md`" — 7 false positives on hwaro's own docs site. Per-language index spellings (`index.ko.md`, `_index.zh-tw.md`) now count as index pages, matched against the site's declared language codes
- **`hwaro doctor --fix` silently did nothing when `config.toml` is a symlink.** The atomic rename replaced the link with a regular file and left the real config (`shared/config.toml`, a common monorepo layout) untouched, while reporting the fix as applied. The rewrite now resolves the link first, and the temp file is per-process so two concurrent runs can't collide on it
- `hwaro doctor` no longer reports a remote `[og] default_image` as a missing file. Absolute (`https://…`), protocol-relative (`//cdn…`) and `data:` values are supported by `og:image` and can't be resolved against `static/`, so every CDN-hosted OG image drew a spurious warning; the same rule now covers `[pwa] precache_urls`, which only skipped literal `http://`/`https://`
- `hwaro doctor` reports a missing `content/` directory instead of scanning nothing and calling the site clean — the build already says the run produced no pages
- `[doctor] ignore` entries that match no rule id are reported as having no effect. A typo silenced nothing, in silence — made worse by the docs, which listed three ids (`base-url-scheme`, `template-unclosed-block`, `template-mismatched-vars`) that no longer exist and omitted twelve that do. The rule-id table is now complete
- `hwaro doctor`'s `title-default` advisory fires on `My Hwaro Site`, the placeholder every `hwaro init` writes. It only matched `Hwaro Site` — the internal fallback for an *absent* `title` — so the check never fired on a real scaffolded site and the placeholder shipped into `<title>`, OG tags and feeds. An absent `title` now says so instead of naming a placeholder value the file doesn't contain
...
- `hwaro build` tunes the Boehm GC at startup: `GC_MARKERS=1` and `GC_INITIAL_HEAP_SIZE=256M`, each honoured only when the variable is not already exported. Boehm's defaults — one marker thread per core and a tiny initial heap — are the worst shape for a short-lived, allocation-heavy build process, and the tuning measured 3-5x faster on template-heavy sites (64s → 14s on a 3000-page corpus on a 14-core Mac, 105s → 31s on Linux/musl) with every corpus at least slightly faster on both platforms. Costs a higher peak RSS during the build (docs site: 313MB → 390MB). Output is byte-identical. The heap presize steps aside for `--memory-limit` / `HWARO_MEMORYLIMIT`, other commands (`serve` included) keep stock GC behavior, and the injected variables are scrubbed from the environment before user hooks or deploy commands are spawned
