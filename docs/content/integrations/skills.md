...
- **Design Read first:** reads your brief and declares a one-line design
  ...
  round only when your intent is ambiguous, and runs a full taste
...
- **Hwaro mechanics:** Crinja template structure, the three CSS-delivery options
  ...
  and `IntersectionObserver`, with no framework required.
...
project-specific conventions. Agents should read `AGENTS.md` first, since it can
