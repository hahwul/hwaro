...
- 페이지는 게시 주소를 유지합니다. Hugo `url`과 Jekyll의 리터럴 `permalink`(`:placeholder` 패턴이 아닌 것)는 `path`가 됩니다. `.html` 주소는 확장자 없는 `path`와 옛 주소의 별칭이 되며, Jekyll `redirect_from` 항목은 `aliases`가 됩니다.
- Hugo: JSON 프론트 매터도 TOML·YAML처럼 읽으며, `slug`가 있는 리프 번들은 슬러그 디렉터리 아래 번들(`posts/<slug>/index.md`)로 씁니다.
- Jekyll: `last_modified_at`은 `updated`가 되고, `image: {path: …}` 형식은 `image`가 됩니다.
