...
| 필드 | 필수 | 의미 |
|------|------|------|
| `key` | 예 | `site.data` 아래의 이름. 문자, 숫자, `_`, `-`만 허용하며 각 키는 한 번만 선언할 수 있습니다. |
| `url` | 예 | 절대 `http://` 또는 `https://` URL. 다른 스킴은 설정을 읽는 시점에 거부됩니다. |
| `format` | 아니요 | `json`, `toml`, `yaml`, `csv`. 생략하면 응답의 `Content-Type`, 그다음 URL의 파일 확장자에서 추론합니다. 둘 다 실패하면 `format`을 명시하라는 안내와 함께 가져오기가 실패합니다. |
...
**환경 변수.** `url`과 `headers` 값의 `${VAR}`는 환경 변수로 치환됩니다. 참조한 변수가 설정되어 있지 않으면 변수 이름을 명시한 오류와 함께 설정 로드가 실패합니다(`config.toml`의 다른 곳에서는 경고에 그칩니다). 기본값이 필요하면 `${VAR:-default}`를 쓰세요.

**캐싱.** 페이로드는 `.hwaro/remote_data/` 아래에 캐시됩니다. `data/`를 비롯해 `hwaro serve`가 감시하는 어떤 디렉터리에도 속하지 않으므로, TTL 안의 serve 재빌드는 API를 다시 호출하는 대신 캐시를 재사용하고, 캐시 기록이 재빌드를 유발하지도 않습니다. 이 캐시는 `hwaro build --full`에도 지워지지 않으며(페이지 캐시만 초기화됩니다), `on_error = "warn-and-use-cache"`와 함께 오프라인 빌드를 가능하게 하는 것도 이 캐시입니다. `.hwaro/`는 `.gitignore`에 추가하세요. 긴 `hwaro serve` 세션 중에는 TTL이 만료된 뒤 첫 전체 재빌드에서 소스를 다시 가져옵니다. 페이로드가 바뀌면 `data/` 파일을 편집했을 때와 같은 방식으로 캐시된 페이지가 무효화됩니다.
