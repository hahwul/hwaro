...
| `highlighted` | 서버 모드로 구문 강조된 본문(hljs 클래스 span). `[highlight] mode`가 `"server"`가 아니거나, 강조가 꺼져 있거나, 해당 언어의 렉서가 없으면 빈 문자열. `{hide_lines=…}` 펜스 옵션은 이미 적용된 상태로, 서버 모드에서 숨긴 줄은 템플릿(그리고 `code`)에 절대 도달하지 않음 |
...
다음 템플릿들은 hwaro의 기본 출력을 그대로 재현하므로 수정의 출발점으로 쓰기 좋습니다.
...
이제 마크다운의 모든 `![alt](src "caption")`이 캡션 달린 `<figure>`로 렌더링되므로 콘텐츠 파일마다 이미지 마크업을 따로 쓸 필요가 없습니다.
