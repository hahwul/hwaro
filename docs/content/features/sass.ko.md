...
`@extend`, 컬러 값과 `color.*` 함수, 단위 변환(`px`↔`cm`), `@at-root (with: ...)` 쿼리, `@forward ... with (...)`, `@content(args)` / `using`, `math.random` / `unique-id()`(빌드는 결정적이어야 합니다), 중첩 속성(`font: { family: ... }`), 들여쓰기 방식의 `.sass` 문법, 소스맵은 지원하지 않습니다.
