...
헤딩이나 인라인 이미지에 붙는 pandoc 스타일 속성 블록입니다. [커스텀 헤딩 ID](/ko/writing/pages/#커스텀-헤딩-id) 축약 문법을 일반화한 것으로, 클래스와 임의 속성까지 설정할 수 있고 이미지에도 적용됩니다. 인용문 안의 헤딩에도 속성을 적용할 수 있습니다.
...
코드 스팬, 코드 블록, 원시 HTML, 수식 본문은 절대 다시 쓰지 않습니다. `<pre>`, `<script>`, `<style>`, `<textarea>` 같은 원시 HTML 코드 블록의 본문도 마크다운 확장 처리에서 리터럴로 유지됩니다(그 안의 숏코드는 그대로 확장됩니다). 표 셀, 정의 본문, 각주 본문은 미리 렌더링된 HTML이라 그 내부에는 스마트 문장부호가 적용되지 않습니다. `markdownify` 템플릿 필터는 사이트 설정을 따릅니다.
...
원시 HTML 앵커도 속성명 대소문자와 작은따옴표/큰따옴표 사용 여부에 관계없이 처리됩니다.
