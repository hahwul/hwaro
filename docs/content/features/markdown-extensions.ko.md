...
코드 스팬, 코드 블록, 원시 HTML, 수식 본문은 절대 다시 쓰지 않습니다. `<pre>`, `<script>`, `<style>`, `<textarea>` 같은 원시 HTML 코드 블록의 본문도 마크다운 확장 처리에서 리터럴로 유지됩니다. 표 셀, 정의 본문, 각주 본문은 미리 렌더링된 HTML이라 그 내부에는 스마트 문장부호가 적용되지 않습니다. `markdownify` 템플릿 필터는 사이트 설정을 따릅니다.
