...
가장 흔한 경우 — GET 요청 하나의 페이로드를 `site.data`에 넣는 것 — 라면 훅이 필요 없습니다. `config.toml`에 [`[[data.remote]]` 소스](/ko/features/remote-data/)를 선언하면 Hwaro가 빌드당 한 번 가져오며, 디스크 캐싱과 명시적인 오류 처리가 내장되어 있습니다.
