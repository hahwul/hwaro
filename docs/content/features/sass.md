+++
title = "Sass/SCSS"
description = "Built-in SCSS compilation in pure Crystal, no external tools"
weight = 16
toc = true
+++
...
- **Value contexts are lenient.** A declaration or variable value is evaluated only when it visibly computes something: an operator between numbers, or a call to a known function. Anything else, and anything that *fails* to evaluate (`$a + 2em` with incompatible units, `min(100% - 10px, 20rem)`), keeps its verbatim text exactly as before. Existing stylesheets compile byte-identically.
...
- `/` follows the classic Sass division rule (dart-sass 1.x): it divides when an operand is a variable, a known function call, or parenthesized, or when the slash sits in a computing context (adjacent arithmetic, a Sass function argument, a variable declaration, interpolation, control flow). Literal-only slashes (`font: 12px/1.5`, `grid-area: 1 / 2`) stay verbatim **with the author's spacing**, whereas dart-sass reprints them compacted (`1/2`).
...
- `and`/`or` in *value* positions only operate on real booleans, so `font-family: Franklin and Marshall` stays text. Conditions have full Sass truthiness.
...
- A `calc()` whose contents fold to a single static number is simplified the way dart-sass does: `calc(10px + 5px * 2)` → `20px`, `calc(9 / 21 * 100%)` → `42.8571428571%`, nested `calc`/`min`/`max`/`clamp` included (`clamp` bounds win at exact boundaries, unit spelling included). Anything not fully foldable keeps its verbatim text: `calc(100% - 20px)`, `var()`, interpolated `calc(#{…})`, unitless↔united addition or clamp, `+`/`-` without surrounding whitespace (invalid CSS that dart-sass errors on), and division producing NaN. `@supports (width: calc(…))` never folds, because the query tests calc support itself. A slash after a literal `calc()` (`font: calc(16px)/1.5`) stays verbatim like every literal slash.
...
- `&` substitution is textual, so `&__elem` concatenates without validating the compound selector.
